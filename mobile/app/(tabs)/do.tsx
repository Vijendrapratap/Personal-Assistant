/**
 * Do Screen (Expo Router)
 *
 * Tasks + Habits unified view with conversation-first interface.
 * Adapted from src/screens/DoScreen for file-based routing.
 */
...
import { TaskCard } from '../../src/components/tasks';
import { HabitChip } from '../../src/components/habits';
import { ConversationInput } from '../../src/components/input';
import { tasksApi, habitsApi, projectsApi, Task, Habit, Project } from '../../src/api/services';
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.bg }]}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          >
            {(['today', 'all', 'in_progress', 'overdue'] as FilterType[]).map((f) => (
              <TouchableOpacity
                key={f}
                style={[
                  styles.filterTab,
                  {
                    backgroundColor:
                      filter === f ? theme.colors.primary : theme.colors.bgSurface,
                    borderColor:
              ...
              >
                ...
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        filter === f ? 'rgba(255,255,255,0.2)' : theme.colors.bgHover,
                    },
                  ]}
                >
                  <Text
                    style={[
                      ...
                      { color: filter === f ? '#fff' : theme.colors.textTertiary },
                    ]}
                  >
                    {getFilterCount(f)}
                  </Text>
        ...
        {tasks.filter((t) => t.status === 'in_progress').length > 0 && filter !== 'in_progress' && (
          <View style={styles.section}>
            ...
            {tasks
              ...
              .slice(0, 1)
              .map((task) => (
                <Card key={task.task_id} variant="primary" style={styles.activeTaskCard}>
