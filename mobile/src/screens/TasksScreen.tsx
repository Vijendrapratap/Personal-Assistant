...
export default function TasksScreen({ navigation }: Props) {
    ...
    return (
        <View style={styles.container}>
            ...
            >
                {(['pending', 'in_progress', 'blocked', 'completed'] as FilterType[]).map((f) => (
                    <TouchableOpacity
                        key={f}
                        ...
                        <View style={styles.actionRow}>
                            {task.status === 'pending' && (
                                <TouchableOpacity
                                    ...
                                    onPress={() => handleStartTask(task.task_id)}
                                >
                            ...
                            {(task.status === 'pending' || task.status === 'in_progress') && (
                                <>
                                    <TouchableOpacity
                                        ...
                                        onPress={() => handleBlockTask(task.task_id)}
                                    >
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.projectSelector}>
                            <TouchableOpacity
                                style={[
                                    ...
                                    !newTask.project_id && styles.projectOptionActive
                                ]}
                                onPress={() => setNewTask({ ...newTask, project_id: '' })}
                            >
                                <Text style={[styles.projectOptionText, !newTask.project_id && { color: '#1a73e8' }]}>
                                    None
                                </Text>
                            </TouchableOpacity>
                            {projects.map((project) => (
                                <TouchableOpacity
                                    ...
                                    style={[
                                        styles.projectOption,
                                        newTask.project_id === project.project_id && styles.projectOptionActive
                                    ]}
                                    onPress={() => setNewTask({ ...newTask, project_id: project.project_id })}
                                >
                                    <Text style={[
                                        styles.projectOptionText,
                                        newTask.project_id === project.project_id && { color: '#1a73e8' }
                                    ]}>
                                        {project.name}
                                    </Text>
                                </TouchableOpacity>
